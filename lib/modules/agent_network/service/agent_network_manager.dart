...
$message''';
  ...
  /// Generates loading words in the background and updates the provider.
  void _generateLoadingWords(Ref ref, String userMessage) async {
    // Don't clear existing words - keep showing them until new ones arrive
    // Generate creative loading words with Haiku
    final words = await LoadingWordService.generateWordsWithHaiku(userMessage);
    if (words != null) {
      ref.read(dynamicLoadingWordsProvider.notifier).state = words;
    }
  }
}

/// Provider for dynamic loading words generated by Haiku.
/// Updates when a new message is sent.
final dynamicLoadingWordsProvider = StateProvider<List<String>?>((ref) => null);
