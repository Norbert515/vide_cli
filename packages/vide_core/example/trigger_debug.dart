...
void main() async {
  ...
  try {
    ...
    final session = await core.startSession(VideSessionConfig(
      ...
      initialMessage: 'Say "Hello, triggers are working!" and nothing else. This is a test.',
      team: 'enterprise',
    ));
    ...
    session.events.listen((event) {
      ...
      if (event is MessageEvent) {
        ...
        if (content.isNotEmpty) {
          print('   Content: ${content.substring(0, content.length.clamp(0, 100))}${content.length > 100 ? "..." : ""}');
        }
