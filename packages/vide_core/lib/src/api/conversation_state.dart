/// Conversation state accumulator for the VideCore API.
///
...
/// A piece of content within a conversation message.
///
/// This represents either text content or a tool invocation with its result.
...
///
/// Use [ConversationStateManager] to create and manage these.
class AgentConversationState {
  ...
  String? _currentMessageEventId;

  /// Internal: pending tool uses waiting for results.
  final Map<String, int> _pendingToolMessageIndex = {};
  final Map<String, int> _pendingToolContentIndex = {};
}
...
/// Example:
/// ```dart
/// final stateManager = ConversationStateManager();
///
...
class ConversationStateManager {
  ...
  void _handleMessage(MessageEvent event) {
    ...
    if (state._currentMessageEventId != event.eventId) {
  ...
  void _handleToolResult(ToolResultEvent event) {
    ...
    final messageIndex = state._pendingToolMessageIndex.remove(event.toolUseId);
    final contentIndex = state._pendingToolContentIndex.remove(event.toolUseId);
