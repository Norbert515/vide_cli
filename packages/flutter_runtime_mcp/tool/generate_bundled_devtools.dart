...
const _sourceFiles = [
  ...
  'lib/src/navigation_extension.dart',
  'lib/src/error_extension.dart',
  'lib/src/device_presets.dart',
  'lib/src/device_size_state.dart',
  'lib/src/device_size_extension.dart',
  'lib/src/animation_extension.dart',
  'lib/src/theme_extension.dart',
  'lib/src/locale_extension.dart',
...
const _renames = {
  ...
  'registerNavigationExtension': '_registerNavigationExtension',
  'registerErrorExtension': '_registerErrorExtension',
  'registerDeviceSizeExtension': '_registerDeviceSizeExtension',
  'registerAnimationExtension': '_registerAnimationExtension',
  'registerThemeExtension': '_registerThemeExtension',
  'registerLocaleExtension': '_registerLocaleExtension',
  'DeviceSizeState': '_DeviceSizeState',
  'DeviceSizeSettings': '_DeviceSizeSettings',
  'deviceSizeState': '_deviceSizeState',
  'devicePresets': '_devicePresets',
  'ThemeModeOverride': '_ThemeModeOverride',
...
final _exportPatterns = [RegExp(r"^export\s+[^;]+;.*$", multiLine: true)];
...
void main() async {
  ...
  if (scriptPath.contains('tool/generate_bundled_devtools.dart')) {
    ...
    runtimeAiDevToolsDir = path.join(
      path.dirname(flutterRuntimeMcpDir),
  ...
  if (!Directory(runtimeAiDevToolsDir).existsSync()) {
    print(
      'Error: runtime_ai_dev_tools directory not found at: $runtimeAiDevToolsDir',
    );
    exit(1);
  }
  if (!Directory(flutterRuntimeMcpDir).existsSync()) {
    print(
      'Error: flutter_runtime_mcp directory not found at: $flutterRuntimeMcpDir',
    );
    exit(1);
  }
  ...
  print(
    'Will write output to: $flutterRuntimeMcpDir/lib/src/bundled_devtools_code.dart',
  );
  ...
  final outputPath = path.join(
    flutterRuntimeMcpDir,
    'lib',
    'src',
    'bundled_devtools_code.dart',
  );
...
String _generateOutput(Map<String, String> processedFiles) {
  ...
  if (codeContent.contains("'''")) {
    ...
    print(
      "Warning: Source code contains triple quotes, using alternative escaping",
    );
